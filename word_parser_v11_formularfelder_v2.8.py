# ============================================================
# word_parser_v11_formularfelder.py
# V11 Systembewertung Extraktor - 2.8
...
def text_matches(search_term, cell_text):
    ...
    return bool(s_clean and c_clean and (s_clean in c_clean or c_clean in s_clean))
...
def parse_betrieb(raw):
    ...
    if "/" in raw:
        ...
        nicht_gebaeude = [p for p in parts if not re.match(r'^[A-Z]\d+$', p)]
...
def parse_sw_version(text):
    ...
    for pattern in [r'(800xA)', r'(Freelance)', r'(WinCC)', r'(PCS\s*7)', r'(SCADA\s*\S+)', r'(TIA[-\s]*Portal)']:
...
def parse_gkat_subtypen(text):
    result = {}
    subtypen = {"GKATB1": r'B1', "GKATB2": r'B2', "GKATB3": r'B3', "GKATC1": r'C1', "GKATC2": r'C2', "GKATC3": r'C3'}
    for col, pattern in subtypen.items():
        result[col] = "r" if re.search(pattern, text or "") else "c"
    return result

def parse_gxp_einfluss(text):
    result = {"GxP_Produktqualitaet": "", "GxP_Patientensicherheit": "", "GxP_Datenintegritaet": ""}
    if not text or not text.strip():
        return result
    t = text.strip()
    hat_produkt = "produktqualität" in t.lower()
    hat_patient = "patientensicherheit" in t.lower()
    hat_daten = ("datenintegrität" in t.lower() or "datenintegrit" in t.lower())
    if hat_produkt or hat_patient or hat_daten:
        hat_doppelpunkt = bool(re.search(r'(?i)(produktqualität\s*:|patientensicherheit\s*:|datenintegrit[äa]t\s*:)', t))
        if not hat_doppelpunkt:
            result["GxP_Produktqualitaet"] = t
            return result
        teile = re.split(r'(?i)(produktqualität\s*[:\-]?|patientensicherheit\s*[:\-]?|datenintegrit[äa]t\s*[:\-]?)', t)
        aktueller_key = None
        for teil in teile:
            ...
            if tl == "produktqualität":
                ...
                if cleaned:
                    result[aktueller_key] = (result[aktueller_key] + " " + cleaned).strip()
    else:
        result["GxP_Produktqualitaet"] = t
    return result
...
def berechne_systemtyp_ce(data):
    if any(data.get(f) == "r" for f in ["Subtyp_PCS", "Subtyp_LCE", "Subtyp_EE"]):
        return "r"
    return "c"
        ...
        for row in table.rows:
            ...
            if "system id" in label.lower() and "mlcs" in label.lower():
                ...
                for i in range(len(cells)-1, 0, -1):
                    ...
                    val = re.sub(r"\(MLCS.*?\)", "", val, flags=re.IGNORECASE).strip()
    ...
    for table in doc.tables:
        for row in table.rows:
            ...
            if len(cells) < 2:
                continue
            label = get_cell_text(cells[0])
            if ("anlagen-id" in label.lower() or "equipment-nr" in label.lower()):
                seen = set()
                for i in range(len(cells)-1, 0, -1):
                    val = get_cell_text(cells[i]).strip()
                    if val in seen:
                        continue
                    seen.add(val)
                    val = re.sub(r"anlagen-ids/equipment-nr\./qc-id:?", "", val, flags=re.IGNORECASE).strip()
                    if val and not _is_boilerplate(val):
                        return val
    return None
            ...
            if "schnittstelle zu anderen" in label.lower():
                ...
                for i in range(len(cells)-1, 0, -1):
                    ...
                    if val in seen:
                        continue
                    seen.add(val)
                    val = re.sub(r"schnittstelle zu anderen.*?systemen:?", "", val, flags=re.IGNORECASE).strip()
                    if val and not _is_boilerplate(val):
                        return val
    return None
    ...
    for table in doc.tables:
        for row in table.rows:
            ...
            if len(cells) < 1:
                continue
            label = get_cell_text(cells[0])
            if ("begründung" in label.lower() and "gxp" in label.lower()):
                if ":" in label:
...
def extract_neuerstellung(doc):
    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
                text = get_cell_text(cell)
                if ("neuerstellung" in text.lower() and "änderung" in text.lower()):
                    checkboxes = get_checkboxes_from_cell(cell)
                    if len(checkboxes) >= 2:
                        return {"Neuerstellung": checkboxes[0]["state"], "Revisioniert": checkboxes[1]["state"]}
    return {}

def extract_gxp_relevan_bc(doc):
    result = {}
    keywords_gxp = ["gxp relevanz", "gxp-relevanz"]
    keywords_bc = ["business kritikalität", "geschäftskritisch", "business kritisch"]
    for table in doc.tables:
        rows = list(table.rows)
        for row_idx, row in enumerate(rows):
            for cell in row.cells:
                text = get_cell_text(cell).lower()
                if ("GxP_Relevan_JA" not in result and any(k in text for k in keywords_gxp)):
                    for next_row in rows[row_idx+1:row_idx+6]:
                        for nc in next_row.cells:
                            nc_text = get_cell_text(nc).strip().lower()
                ...
                if ("BCkritisch" not in result and any(k in text for k in keywords_bc)):
                    for next_row in rows[row_idx+1:row_idx+6]:
                        for nc in next_row.cells:
                            nc_text = get_cell_text(nc).strip().lower()
                            nc_text_clean = re.sub(r"^[rc]\s+", "", nc_text).strip()
...
def extract_text_fields(doc):
    ...
    for table in doc.tables:
        for row in table.rows:
            ...
            for search_term, excel_col in TEXT_FIELD_MAPPING_V11.items():
...
def extract_checkboxes_formularfelder(doc):
    ...
    for table_idx, table in enumerate(doc.tables):
        if table_idx not in CHECKBOX_MAPPING_V11:
            continue
        table_mapping = CHECKBOX_MAPPING_V11[table_idx]
...
def extract_history(doc):
    ...
    for table in doc.tables:
        for row in table.rows:
            ...
            if ("version" in col0.lower() and "freigabe" in col0.lower()):
                continue
...
def parse_systembewertung_v11(docx_path):
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        print("="*55)
        print("  V11 Systembewertung Extraktor")
        print("="*55)
    ...
    try:
        data = parse_systembewertung_v11(docx_path)
        if not data:
            ...
            validiere_kategorien(data, VALIDATION_KATEGORIEN_V11)
